...
def default_shared_state():
    return {"facilitator_code":"FAC2026","team_codes":{"team_red":"RED2026","team_blue":"BLUE2026"},"timer_minutes":12,
            "competition_end_ts":None,"timer_running":False,
...
def restart_competition(state):
    state["competition_end_ts"]=None; state["timer_running"]=False
    for tk in ("team_red","team_blue"):
        name=state["teams"][tk]["name"]; state["teams"][tk]=default_team_state(name, tk)
    save_state(state)

def remaining_seconds(state):
    if not state["timer_running"] or state["competition_end_ts"] is None: return int(state["timer_minutes"]*60)
...
def render_clock(seconds):
    ...
    <script>
    let total = {seconds}; const el = document.getElementById("clock");
    function tick(){{ if(total < 0) return; const m = String(Math.floor(total/60)).padStart(2,'0'); const s = String(total%60).padStart(2,'0'); el.textContent = `${{m}}:${{s}}`; total -= 1; }}
...
def render_team_screen(team_key):
    ...
    with top2: render_clock(remaining_seconds(state))
    with top3:
...
def render_facilitator():
    ...
    with top_left:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
    with top_mid: render_clock(remaining_seconds(state))
    ...
    with left:
        ...
        state["teams"]["team_red"]["name"]=st.text_input("Red team name", value=red["name"]); state["teams"]["team_blue"]["name"]=st.text_input("Blue team name", value=blue["name"]); state["timer_minutes"]=st.number_input("Timer (minutes)", min_value=1, max_value=60, value=int(state["timer_minutes"]), step=1)
        if st.button("Save names and timer", use_container_width=True): save_state(state); st.success("Setup saved.")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if st.button("Save access codes", use_container_width=True): save_state(state); st.success("Codes saved.")
        st.markdown('</div>', unsafe_allow_html=True)
    with right:
        ...
        c1,c2=st.columns(2)
        with c1:
            if st.button("Start / restart timer", use_container_width=True): state["competition_end_ts"]=time.time()+int(state["timer_minutes"]*60); state["timer_running"]=True; save_state(state); st.rerun()
        with c2:
            if st.button("Stop timer", use_container_width=True): state["timer_running"]=False; save_state(state); st.rerun()
        if st.button("Reset whole competition", use_container_width=True): restart_competition(state); st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
...
def render_projection():
    ...
    with t2: render_clock(remaining_seconds(state))
    cols=st.columns(2)
