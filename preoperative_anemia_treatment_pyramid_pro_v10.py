...
CASES = [
    ...
    {"case":5,"title":"Challenge 5","type":"sequence","icon":"🔢","scenario":"63-year-old woman, Hb 9.5 g/dL, normal ferritin, low TSAT, functional iron deficiency. Elective surgery in 2 weeks.","question":"Choose the correct management sequence.","cards":["Measure ferritin","Calculate iron deficit","IV iron","Re-evaluate Hb","Oral iron","ESA"],"correct_sequence":["Measure ferritin","Calculate iron deficit","IV iron","Re-evaluate Hb"],"clue":"Hint: First confirm iron status, then estimate replacement need, then choose the fastest treatment, then reassess response.","piece":"Pyramid piece: Clinical sequencing mastery piece.","explanation":"In functional iron deficiency with limited time before surgery, the logical sequence is to assess iron status, estimate the iron deficit, use IV iron for rapid optimisation, and then re-evaluate haemoglobin response."},
...
.card-pick:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(15,23,42,.12); }
...
def render_case_header(case, colors):
    ...
    icon = case.get("icon","🩺")
    st.markdown(f"""
    <div class="glass-card" style="border-left:6px solid {colors['primary']};animation:fade-in .5s ease;">
      <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
        <span style="font-size:38px;">{icon}</span>
        <div>
          <div style="font-size:28px;font-weight:900;color:#0f172a;">{case['title']}</div>
          {'<span class="case-badge" style="background:'+badge_color+'22;color:'+badge_color+';border:1px solid '+badge_color+'44;">'+badge_label+'</span>' if badge_label else ''}
        </div>
      </div>
      <div style="margin-top:14px;padding:14px 16px;background:#f8fafc;border-radius:14px;border:1px solid #e2e8f0;">
        ...
        <div style="font-size:15px;color:#1e293b;line-height:1.6;">{case['scenario']}</div>
      </div>
...
def render_mcq_case(case, team, team_key, colors):
    ...
    if team["answered_correctly"]:
        ...
        st.markdown(f'<div class="feedback-explain"><b>📖 Why this is correct:</b> {case["explanation"]}</div>', unsafe_allow_html=True)
...
def render_projection():
    ...
    with action_left:
        if st.button("🔄 Refresh status", key="projection_refresh", use_container_width=True): st.rerun()
    with action_right:
